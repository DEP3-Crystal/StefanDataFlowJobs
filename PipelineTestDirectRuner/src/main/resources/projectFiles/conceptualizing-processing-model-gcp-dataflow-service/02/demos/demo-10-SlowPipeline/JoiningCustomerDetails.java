...
public class JoiningCustomerDetails {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(JoiningCustomerDetails.class);
    ...
    static void performJoin(GcsOptions options) {
        ...
        PCollection<KV<String, CoGbkResult>> joined = KeyedPCollectionTuple.of(incomeTag, customersIncome)
                .and(scoreTag, customersScore).apply(CoGroupByKey.create());

        joined.apply("Joining", ParDo.of(new DoFn<KV<String, CoGbkResult>, TableRow>() {
            ...
            public void processElement(@Element KV<String, CoGbkResult> element, OutputReceiver<TableRow> out) {
                ...
                out.output(new TableRow().set(FormatSchema.CUSTOMER_ID, id).set(FormatSchema.CUSTOMER_INCOME, income)
                        .set(FormatSchema.CUSTOMER_SCORE, spendingScore));
            }
        })).apply("WriteDataToBigQuery", BigQueryIO.writeTableRows()
                .to("loony-dataflow-review:results." + options.getTableName()).withSchema(FormatSchema.getSchema()));
    ...
    private static class IdIncomeKVFn extends DoFn<String, KV<String, Integer>> {
        ...
        public void processElement(@Element String element, OutputReceiver<KV<String, Integer>> out) {
            ...
            LOG.info("Slowing Pipeline by adding delay");
